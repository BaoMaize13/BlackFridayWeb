...
exports.up = async function up(knex) {
  ...
  await knex.schema.renameTable("purchase_logs", "purchase_logs_phase10");
  await createPurchaseLogsTable(knex, Object.values(PURCHASE_LOG_ACTIONS));

  await knex("purchase_logs").insert(
    knex("purchase_logs_phase10").select(
      "id",
      "request_id",
      "server_id",
      "product_id",
  ...
  await knex.schema.dropTable("purchase_logs_phase10");
};
...
exports.down = async function down(knex) {
  ...
  await dropPurchaseLogIndexes(knex);
  await knex.schema.renameTable("purchase_logs", "purchase_logs_phase11");
  await createPurchaseLogsTable(knex, Object.values(PURCHASE_LOG_ACTIONS));

  await knex("purchase_logs").insert(
    knex("purchase_logs_phase11").select(
      "id",
      "request_id",
      "server_id",
      "product_id",
  ...
  await knex.schema.dropTable("purchase_logs_phase11");
};
